...
public class ManagerServiceImpl extends ServiceImpl<IManagerMapper, Manager> implements IManagerService {
    ...
    public List<Map<String, Object>> getAllStuWithManagerByPage(String studentName, int managerId, int periodNo,
        ...
        String sql = "SELECT e.student_id,student_name,manager_id,score,quality_name,e.overall_score dept," +
                "evaluation_form_dept,eos.overall_score school,q_period,e_period," +
                dynamicSequence.get(dynamicSequence.size() - 1) +
                " FROM(" +
                " SELECT d.student_id,student_name,manager_id,score,quality_name,overall_score," +
                "evaluation_form_dept,d.period_no q_period,eod.period_no e_period" +
                " FROM(" +
                " SELECT c.student_id,student_name,manager_id,score,quality_name,period_no" +
                " FROM(" +
                " SELECT b.student_id,student_name,manager_id,score,quality_id,period_no" +
                " FROM(" +
                " SELECT student_id,student_name,manager_id" +
                " FROM(" +
                " SELECT student_id, student_name, d.dept_no FROM student s LEFT JOIN dept d ON d.dept_no = s.dept_no ) a" +
